  ...
  try {
    ...
    if (minRent !== undefined || maxRent !== undefined) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Minimum rent cannot exceed maximum rent" });
      }
    }
    ...
    if (availableBeds !== undefined) {
      ...
      if (!Number.isInteger(minimumBeds) || minimumBeds < 0) {
        return res
          .status(400)
          .json({ message: "Invalid available beds value" });
      }
    ...
    // -------------------------
      ...
      [pgs, total] = await Promise.all([
        PgModel.find(filter).sort("-createdAt").skip(skip).limit(limitNumber),
...
const getPGByIdController = async (req, res) => {
  try {
    const pg = await PgModel.findById(req.params.id).populate(
      "owner",
      "name phone email",
    );
...
const createPgController = async (req, res) => {
  try {
    ...
    if (
      ...
      !images.every((image) => isOwnerCloudinaryImage(image, owner))
    ) {
      return res
        .status(400)
        .json({ message: "Images must be uploaded through StayFinder" });
    }
    ...
    const availableBedsNumber =
      availableBeds !== undefined ? Number(availableBeds) : totalBedsNumber;
  ...
  try {
    ...
    if (req.body.images !== undefined) {
      if (
        ...
        !req.body.images.every((image) => isOwnerCloudinaryImage(image, userId))
      ) {
        return res
          .status(400)
          .json({ message: "Images must be uploaded through StayFinder" });
    ...
    if (req.body.images !== undefined) {
      const retainedPublicIds = new Set(
        updatedPg.images.map((image) => image.publicId),
      );
      ...
      try {
        ...
        console.error(
          "Failed to remove old Cloudinary images:",
          cloudinaryError,
        );
      }
    }
