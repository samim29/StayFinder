...
const requiredString = (fieldName, label, options = {}) => {
  const { minLength, maxLength, pattern, allowedValues } = options;
...
const optionalArray = (fieldName, label, { allowedValues } = {}) => {
  return (payload) => {
    const value = payload[fieldName];
    ...
    if (Array.isArray(allowedValues)) {
      const invalidValue = value.find((item) => !allowedValues.includes(item));
